...
public class UserLoginActivity extends AppCompatActivity {
    ...
    private void loginUser() {
        ...
        if (TextUtils.isEmpty(email) || TextUtils.isEmpty(password)) {
            Toast.makeText(UserLoginActivity.this, "Please enter all details", Toast.LENGTH_SHORT).show();
            return;
        }
                    ...
                    if (task.isSuccessful()) {
                        mAuth.getCurrentUser().sendEmailVerification().addOnCompleteListener(verificationTask -> {
                            if (verificationTask.isSuccessful()) {
                                Toast.makeText(UserLoginActivity.this, "Verification email sent", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
    ...
    private void resetPassword() {
        ...
        mAuth.sendPasswordResetEmail(email)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(UserLoginActivity.this, "Password reset email sent", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(UserLoginActivity.this, "Error in sending reset email", Toast.LENGTH_SHORT).show();
                    }
                });
    }
